...
def _pretty_link(link: str, target: str, max_link_len: int) -> str:
    return link + (" " * (max_link_len - len(link))) + " -> " + target
...
def setup_dotfiles(dotfiles: Path):
    ...
    # apply symlinks
    if len(links) > 0:
        max_link_len = max(len(str(l)) for l in links)
        for target, link in links.items():
            logger.info(_pretty_link(str(link), str(target), max_link_len))
            shell.try_link(target, link)
    else:
