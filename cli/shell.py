...
def try_link(src: Path, dst: Path):
    """Tries to create a symlink, hard link, or copy in that order"""
    try:
        os.symlink(src, dst, target_is_directory=dst.is_dir())
        return
    except Exception as e:
        if isinstance(e, OSError):
            logging.warning("you may not have permission to create symlinks if Windows")

    logging.info("cannot symlink, defaulting to hard link")
    try:
        os.link(src, dst)
        return
    except Exception as e:
        if isinstance(e, OSError):
            logging.warning("the dst might be on a different filesystem (or some other os error)")

    logging.info("cannot hard, defaulting to copy")
    os.copy2(src, dst)
