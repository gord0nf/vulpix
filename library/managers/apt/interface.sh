...
_list_internal_packages() {
  installed_packages=()
  load_array_by_line installed_packages <"$APT_PACKAGE_LIST"
}

PY_GET_SYSTEM_PACKAGES='from apt import cache
manual = set(
...
_list_all_system_packages() {
  installed_packages=()
  load_array_by_line_from_command installed_packages \
    python3 -c "$PY_GET_SYSTEM_PACKAGES"
}

_apt_packages_install() {
  for package_id in "${APT_PACKAGES[@]}"; do
    apt install --yes "$package_id" || fatal "failed to install '$package_id'"
...
_apt_packages_upgrade() {
  for package_id in "${APT_PACKAGES[@]}"; do
    apt install --yes "$package_id" || fatal "failed to upgrade '$package_id'"
  done
}
_apt_packages_remove() {
  for package_id in "${APT_PACKAGES[@]}"; do
    apt remove --yes "$package_id" || fatal "failed to remove '$package_id'"
    sed -i "/^${package_id}?$/d" "$APT_PACKAGE_LIST"
  done
}
...
install_packages() {
  ...
  for template in "${package_templates[@]}"; do
    run_foreground_task "$(make_task_name 'install' "$template@apt")" \
      _template_install "$template"
  done
  for package_id in "${package_ids[@]}"; do
    APT_PACKAGES=("$package_id")
    run_foreground_task "$(make_task_name 'install' "$package_id?@apt")" \
      _apt_packages_install
  done
}
...
uninstall_packages() {
  ...
  for package_id in "${package_ids[@]}"; do
    APT_PACKAGES=("$package_id")
    run_foreground_task "$(make_task_name 'uninstall' "$package_id?@apt")" \
      _apt_packages_remove
  done
  for template in "${package_templates[@]}"; do
    run_foreground_task "$(make_task_name 'uninstall' "$template@apt")" \
      _template_uninstall "$template"
  done
}
...
update_packages() {
  ...
  for package in "${packages[@]}"; do
    if package_id=$(_package_id_syntax "$package"); then
      APT_PACKAGES=("$package_id")
      run_foreground_task "$(make_task_name 'update' "$package@apt")" \
        _apt_packages_upgrade
    else
      run_foreground_task "$(make_task_name 'update' "$package@apt")" \
        _template_update "$package"
    fi
  done
}
...
reinstall_packages() {
  ...
  for package in "${packages[@]}"; do
    if package_id=$(_package_id_syntax "$package"); then
      APT_PACKAGES=("$package_id")
      run_foreground_task "$(make_task_name 'reinstall' "$package@apt")" \
        _apt_packages_reinstall
    else
      run_foreground_task $(make_task_name 'reinstall' "$package@apt") \
        _template_reinstall "$package"
    fi
  done
}
